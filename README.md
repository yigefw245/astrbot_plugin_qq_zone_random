...
| 命令 | 说明 |
|------|------|
| `/qz status` | 查看 Cookie / g_tk / OneBot 状态 |
| `/qz autobind` | 自动从 OneBot 获取 Cookie |
| `/qz bind <cookie>` | 手动绑定 Cookie |
| `/qz unbind` | 解绑 Cookie |
| `/qz post <内容>` | 发布说说（支持图片） |
| `/qz generate` | AI 生成说说预览（从聊天记录+当前时间） |
| `/qz autopost` | AI 生成并直接发布 |
| `/qz schedule` | 查看/启动/停止定时发送 |
| `/qz next` | 查看下次发送时间 |
| `/qz help` | 帮助 |
...
```
/qz autobind              # 自动绑定 Cookie
/qz status                # 确认 p_skey 正常
/qz post 你好世界         # 发第一条说说
/qz generate              # AI 生成一条试试
/qz schedule start        # 启动定时发送
```
...
| 现象 | 解决 |
|------|------|
| Cookie 绑定失败 | 确认使用 NapCat 或 LLOneBot，执行 `/qz autobind` |
| 发布失败 | Cookie 可能过期，`/qz unbind` → `/qz autobind` |
| g_tk 显示 NO p_skey | Cookie 缺少 p_skey 字段，重新自动绑定 |
| AI 生成空 | 聊天记录不足，先聊几句再试 |
| 定时不工作 | `/qz schedule start` 启动，确认 Cookie 已绑定 |
